    ...
    if (attachment.data && typeof attachment.data !== 'string') {
      if (isBrowser) {
        return new utils.Promise(function (resolve) {
          var origData = attachment.data;
          if (!origData && origData !== '') {
            throw new Error('in preprocessAttachments and data is undefined'+JSON.stringify(doc));
          }
          utils.readAsBinaryString(origData, function (binary) {
            attachment.data = btoa(binary);
            if ((!attachment.data && attachment.data) !== '' || !binary) {
              var errorString = 'in preprocessAttachments after readAsBinaryString and conversion';
              errorString += 'failed:'+typeof attachment.data + ';'+ typeof binary;
              errorString +=JSON.stringify(doc);
