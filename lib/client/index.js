...
function SocketPouch(opts, callback) {
  ...
  function sendMessage(type, args, callback) {
    ...
    socket.send(type + ':' + messageId + ':' + stringArgs, function () {
  ...
  socket.on('message', function (res) {
    ...
    if (messageType === '0') { // error
      ...
      delete api._callbacks[messageId];
      blobUtil.base64StringToBlob(content.data, content.type).then(function (blob) {
        cb(null, blob);
      });
    }
  });
  ...
  api.destroy = utils.adapterFun('destroy', function (callback) {
    sendMessage('destroy', [], function (err, res) {
      ...
      api._close(function (err) {
        ...
        api.emit('destroyed');
        api.constructor.emit('destroyed', api._name); // TODO: Pouch should handle this
        api._close(function () {
          callback(null, res);
        });
      });
