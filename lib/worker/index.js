...
function sendUncaughtError(socketId, data) {
  log(' -> sendUncaughtError', socketId, data);
  self.postMessage({
    type: 'uncaughtError',
    id: socketId,
  ...
  Promise.resolve().then(function () {
    ...
    return db.get(docId, opts).then(function (doc) {
      ...
      return db.getAttachment.apply(db, args).then(function (buff) {
        sendSuccess(socketId, messageId, buff);
      });
    });
  }).catch(function (err) {
    sendError(socketId, messageId, err);
...
function addUncaughtErrorHandler(db, socketId) {
  return db.then(function (res) {
    res.pouch.on('error', function (err) {
      sendUncaughtError(socketId, err);
    });
  });
}
...
function createDatabase(socketId, messageId, args, pouchCreator) {
  ...
  var db = dbs[key];
  if (db) {
    return addUncaughtErrorHandler(db, socketId).then(function () {
      sendSuccess(socketId, messageId, {ok: true, exists: true});
    });
  }
  ...
  addUncaughtErrorHandler(db, socketId).then(function () {
